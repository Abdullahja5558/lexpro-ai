...
const PromptCard = ({ p, isDark, onClick }: any) => (
  ...
  className={`translate-y-3 p-4 md:p-5 rounded-[24px] md:rounded-[28px] border shadow-sm cursor-pointer transition-all flex items-center md:flex-col md:items-start gap-3 md:gap-4 md:min-h-[150px] md:justify-between overflow-hidden ${
    isDark
...
>
  <div
    className={`w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl flex items-center justify-center shrink-0 transition-all ${isDark ? "bg-white text-black" : "bg-[#1a1a1a] text-white"}`}
  >
    {p.icon}
  </div>
  <div className="flex flex-col overflow-hidden w-full">
    <p className="font-bold text-[12px] md:text-[14px] uppercase tracking-wider line-clamp-1 mb-0.5 md:mb-1 leading-tight">
      {p.title}
    </p>
    <p
      className={`text-[11px] md:text-[12px] font-medium line-clamp-2 leading-snug ${isDark ? "text-slate-400" : "text-slate-500"}`}
    >
      {p.desc}
    </p>
  </div>
...
export default function LexProFinal() {
  ...
  const handleStop = () => {
    if (abortControllerRef.current) {
      ...
      setMessages((prev) => [
        ...
        {
          role: "ai",
          text: "⚠️ You stopped the generation.",
          time,
          isStopped: true,
        },
      ]);
  ...
  const handleAsk = async (forcedQuery?: string) => {
    ...
    try {
      ...
      if (!controller.signal.aborted) {
        setMessages((prev) => [
          ...prev,
          { role: "ai", text: data.answer || data.error, time, isNew: true },
        ]);
      }
    } catch (e: any) {
      if (e.name !== "AbortError") {
        setMessages((prev) => [
          ...prev,
          { role: "ai", text: "Something went wrong. Please try again.", time },
        ]);
      }
    } finally {
      if (!controller.signal.aborted) {
  ...
  return (
    <div
      className={`h-[100dvh] w-full transition-colors duration-700 flex flex-col font-sans overflow-hidden relative ${
        isDark
          ? "bg-[#0a0a0a] text-white"
          : "bg-[#f8f2f4] bg-gradient-to-br from-[#f8f2f4] via-[#fcf8f9] to-[#f4ebef] text-slate-900"
      }`}
    >
      ...
      <div className="fixed top-6 left-6 md:top-10 md:left-10 flex items-center gap-3 z-[100] pointer-events-none">
        <div
          className={`w-10 h-10 md:w-12 md:h-12 rounded-xl flex items-center justify-center shadow-lg transition-all ${isDark ? "bg-white text-black" : "bg-[#1a1a1a] text-white"}`}
        >
          <Scale size={20} />
        </div>
        <span
          className={`font-bold text-[14px] md:text-[16px] tracking-tight transition-colors ${isDark ? "text-white" : "text-slate-800"}`}
        >
          Lex Pro
        </span>
      </div>
      ...
      >
        <div
          className={`px-4 py-2 rounded-xl flex items-center gap-2 shadow-lg backdrop-blur-xl border transition-all ${
            isDark
              ? "bg-white/5 border-white/10 hover:bg-white/10 text-white"
              : "bg-white/80 border-white text-slate-800 hover:bg-white"
          }`}
        >
          <UserCircle2 size={18} />
          <span className="text-[12px] md:text-[14px] font-bold uppercase tracking-widest hidden sm:inline">
            About
          </span>
        </div>
      </button>
        ...
        <AnimatePresence mode="wait">
          {messages.length === 0 ? (
            ...
            >
              ...
              <h2
                className={`text-lg md:text-2xl font-semibold mb-2 md:mb-2 ${isDark ? "text-slate-400" : "text-slate-500"}`}
              >
                How can I assist you today?
              </h2>

              <div
                className={`max-w-xs md:max-w-xl mx-auto p-4 md:p-5 rounded-2xl md:rounded-3xl border transition-all ${isDark ? "bg-white/5 border-white/10" : "bg-white border-slate-100"}`}
              >
                <p
                  className={`text-[13px] md:text-[15px] font-medium leading-relaxed ${isDark ? "text-slate-400" : "text-slate-500"}`}
                >
                  Explore{" "}
            ...
            >
              {messages.map((m, i) => (
                <div
                  key={i}
                  className={`flex gap-3 md:gap-5 ${m.role === "user" ? "flex-row-reverse" : "justify-start"}`}
                >
                  <div className="shrink-0">
                    <div
                      className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center border shadow-sm transition-all ${m.role === "user" ? (isDark ? "bg-white border-white text-black" : "bg-[#1a1a1a] border-black text-white") : isDark ? "bg-[#1a1a1a] border-slate-800 text-white" : "bg-white border-slate-200 text-slate-700"}`}
                    >
                      {m.isStopped ? (
                  ...
                  <div
                    className={`flex flex-col ${m.role === "user" ? "items-end" : "items-start"} max-w-[88%] md:max-w-[80%] group`}
                  >
                    <div
                    ...
                    >
                      {m.role === "ai" && !m.isStopped && (
                        <button
                          ...
                          className={`absolute top-2 right-2 p-1.5 rounded-lg transition-all opacity-0 md:group-hover:opacity-100 ${isDark ? "bg-white/10 text-white hover:bg-white/20" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}
                        >
                          {copiedId === i ? (
                      ...
                      {m.role === "ai" && m.isNew && !m.isStopped ? (
                        <Typewriter text={m.text} />
                      ) : (
                        m.text
                      )}
                    </div>
                    <span className="text-[8px] md:text-[9px] font-bold text-slate-400 mt-2 px-1 uppercase tracking-widest">
                      {m.time}
                    </span>
                  </div>
                </div>
      ...
      <footer className="w-full pb-2 md:pb-6 flex flex-col items-center z-[80] px-4 md:px-6 relative shrink-0">
        <AnimatePresence>
          {messages.length === 0 && (
            <motion.div
              ...
              className="w-full max-w-4xl relative mb-4 md:mb-8"
            >
              ...
              <button
                ...
                className={`absolute -right-2 -top-4 md:top-1/2 md:-translate-y-1/2 w-9 h-9 md:w-11 md:h-11 border shadow-xl rounded-full flex items-center justify-center active:scale-90 transition-all z-[90] ${isDark ? "bg-[#1a1a1a] border-slate-800 text-white" : "bg-white border-slate-100 text-slate-600"}`}
              >
                <RefreshCw size={16} />
              </button>
            </motion.div>
        ...
        <div
          className={`w-full max-w-3xl backdrop-blur-3xl border rounded-[25px] md:rounded-[35px] shadow-2xl flex items-center p-1.5 md:p-2 transition-all relative ${isDark ? "bg-black/60 border-white/10" : "bg-white/95 border-white"}`}
        >
          <div className="relative">
            <button
              ...
              className={`p-3 md:p-4 transition-colors ${isDark ? "text-slate-400 hover:text-white" : "text-slate-400 hover:text-black"}`}
            >
              <Settings
                size={18}
                className={
            ...
            <AnimatePresence>
              {isSettingsOpen && (
                <motion.div
                  ...
                  className={`absolute bottom-full left-0 mb-2 w-44 md:w-48 p-2 rounded-[20px] shadow-2xl border backdrop-blur-3xl z-[110] transition-all ${isDark ? "bg-black/80 border-white/10" : "bg-white/90 border-slate-100"}`}
                >
                  <div className="space-y-1">
                    <div className="flex p-1 bg-slate-500/10 rounded-xl mb-2">
                      <button
                        onClick={() => toggleTheme("light")}
                        className={`flex-1 flex items-center justify-center py-1.5 rounded-lg transition-all ${!isDark ? "bg-white shadow-sm text-black" : "text-slate-400"}`}
                      >
                        <Sun size={13} />
                      </button>
                      <button
                    ...
                    {[
                      { id: "Pro", icon: <Crown size={13} /> },
                      { id: "Turbo", icon: <Zap size={13} /> },
                      { id: "Lite", icon: <Cpu size={13} /> },
                    ].map((m) => (
                      <button
                        key={m.id}
                        onClick={() => {
                        ...
                        className={`w-full flex items-center justify-between px-3 py-2 rounded-xl transition-all ${mode === m.id ? "bg-indigo-500 text-white" : isDark ? "text-slate-300 hover:bg-white/5" : "text-slate-700 hover:bg-slate-50"}`}
                      >
          ...
          <div className="flex items-center gap-1 md:gap-2 pr-1 md:pr-2">
            <button
              ...
              className={`h-10 md:h-12 transition-all active:scale-95 shadow-lg flex items-center justify-center gap-2 rounded-full ${loading ? "w-10 md:w-12 px-0 bg-red-500 text-white hover:bg-red-600" : "px-4 md:px-8 " + (isDark ? "bg-white text-black hover:bg-slate-200" : "bg-[#1a1a1a] text-white hover:bg-black")} disabled:opacity-20`}
            >
              {loading ? (
